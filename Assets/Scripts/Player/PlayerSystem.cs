...
public class PlayerSystem : Singleton<PlayerSystem> {
    ...
    public static IReadOnlyList<Player> Players => Instance._players;
    ...
    public void CreatePlayer(ulong id, RobotData robot, string playerName) {
        var settings = GameSystem.Settings;

        var playerArgs = new PlayerArgs {
            RobotData = robot,
            StartingEnergy = settings.StartingEnergy.Value,
            CardsPerTurn = settings.CardsPerTurn.Value,
            RegisterCount = ExecutionPhase.RegisterCount,
            UpgradeSlots = settings.UpgradeSlots.Value,
            Name = playerName,
            ClientId = id
        };
    ...
    public IEnumerable<Player> GetOrderedPlayers() {
